...
def run_algorithm(input_path: Path, commit: str, flags: Iterable[str]) -> Path:
    """
    Run the algorithm with specified input file, commit, and flags.

    Args:
        input_path: Path to the input file
        commit: commit identifier for the algorithm
        flags: flags to pass to the algorithm

    Returns:
        Path to the output file generated by the algorithm
    """
    versioned_input = input_path.with_stem(f"{input_path.stem}_{commit}")
    shutil.copy(input_path, versioned_input)

    cmd = ["python", "main.py", str(versioned_input), f"-{''.join(flags)}"]
    subprocess.run(cmd, cwd=REPO_DIR, check=True)
    output_file = versioned_input.with_stem(f"{versioned_input.stem}_output")

    versioned_input.unlink()
    return output_file
...
def run_and_compare(
    commit_a: str, commit_b: str, input_file: str | Path, flags: Iterable[str]
) -> pd.DataFrame:
    """
    ...
    output_a_path = run_algorithm(input_file, commit_a, flags)

    subprocess.run(["git", "-C", REPO_DIR, "checkout", commit_b], check=True)
    output_b_path = run_algorithm(input_file, commit_b, flags)
