...
def run_algoritmus_from_local(input_path: Path, flags: list[str] | None) -> Path:
...
def compare_outputs(output_file_a: Path, output_file_b: Path, version_a: str, version_b: str) -> pd.DataFrame:
    ...
    return merged_df.loc[merged_df[f"ms_{version_a}"] != merged_df[f"ms_{version_b}"]]
...
class VersionConfig:
    ...
    Args:
        identifier: Either "local" to use the locally installed version, or a git ref
    ...
    def label(self) -> str:
        """Label for the version configuration."""
        return "_".join([self.identifier, str(self.flags)])


def run_algoritmus(version_cfg: VersionConfig, input_path: Path) -> Path:
    """Run algoritmus based on the configuration."""
    if version_cfg.identifier == "local":
        return run_algoritmus_from_local(input_path, version_cfg.flags)
    return run_algoritmus_from_ref(version_cfg.identifier, input_path, version_cfg.flags)
...
def run_and_compare(
    ...
    *,
    ...
    try:
        ...
        output_b_path = run_algoritmus(version_b_cfg, input_path)
